...
func (s *ObjectStore) uploadMultipartPartChunks(ctx context.Context, input UploadPartInput, contentType string) ([]multipartChunk, string, []byte, int64, error) {
	...
	for remaining > 0 {
		...
		limitReader := io.LimitReader(input.Body, thisChunkSize)
		chunkSHAHash := sha256.New()
		tee := io.TeeReader(limitReader, io.MultiWriter(md5Hash, chunkSHAHash))
		...
		// If upload succeeded but didn't read all bytes (e.g. mock or error), we must consume the rest of the limitReader
		// to maintain the stream state and correct chunkSHAHash/md5Hash calculations.
		if _, err := io.Copy(io.Discard, limitReader); err != nil {
			return nil, "", nil, 0, err
		}
		
		chunkSHA := chunkSHAHash.Sum(nil)
...
func (s *ObjectStore) GetObject(ctx context.Context, input GetObjectInput) (io.ReadCloser, ObjectInfo, error) {
	...
	go func() {
		...
		for _, chunk := range selected {
			log.Printf("debug S3 GetObject starting downloadChunk skip=%d take=%d fileID=%q", chunk.Skip, chunk.Take, chunk.FileID)
			reader, err := s.downloadChunk(ctx, chunk.FileID, chunk.Skip, chunk.Take)
			if err != nil {
				log.Printf("debug S3 GetObject downloadChunk failed error=%v", err)
				_ = pw.CloseWithError(err)
				return
			}
			log.Printf("debug S3 GetObject downloadChunk succeeded, copying to pipe")

			if written, err := io.Copy(pw, reader); err != nil {
				log.Printf("debug S3 GetObject io.Copy failed error=%v written=%d", err, written)
				_ = reader.Close()
				_ = pw.CloseWithError(err)
				return
			} else {
				log.Printf("debug S3 GetObject io.Copy completed written=%d", written)
			}
			if err := reader.Close(); err != nil {
				log.Printf("debug S3 GetObject reader.Close failed error=%v", err)
				_ = pw.CloseWithError(err)
				return
			}
		}
		log.Printf("debug S3 GetObject finished all chunks")
	}()
...
func (s *ObjectStore) CopyObject(ctx context.Context, bucket, srcKey, dstKey string) (CopyObjectResult, error) {
	if err := s.HeadBucket(ctx, bucket); err != nil {
		return CopyObjectResult{}, err
	}
	srcMeta, err := s.meta.HeadObject(ctx, bucket, srcKey)
	if err != nil {
		return CopyObjectResult{}, err
	}
	_, err = s.meta.CopyObject(ctx, bucket, srcKey, dstKey, metadata.CopyOptions{Overwrite: true})
	if err != nil {
		return CopyObjectResult{}, err
	}
...
func (s *ObjectStore) putChunked(ctx context.Context, input PutObjectInput, strategy UploadStrategy) (PutObjectResult, error) {
	...
	for remaining > 0 {
		...
		limitReader := io.LimitReader(input.Body, thisChunkSize)
		chunkSHAHash := sha256.New()
		tee := io.TeeReader(limitReader, io.MultiWriter(wholeMD5, wholeSHA, chunkSHAHash))
...
func (s *ObjectStore) downloadChunk(ctx context.Context, fileID string, skip, take int64) (io.ReadCloser, error) {
	release := s.acquire(ctx, s.telegramSem)
	if release == nil {
		return nil, ctx.Err()
	}
	reader, err := s.tg.Download(ctx, telegram.DownloadRequest{
		FileID: fileID,
		Offset: skip,
		Limit:  take,
	})
