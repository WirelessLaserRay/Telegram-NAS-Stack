...
func (c *HTTPClient) Upload(ctx context.Context, request UploadRequest) (UploadedFile, error) {
	...
	if err != nil {
		return UploadedFile{}, fmt.Errorf("%w: raw response: %s", err, string(resp))
	}
...
func (c *HTTPClient) Download(ctx context.Context, request DownloadRequest) (io.ReadCloser, error) {
	values := url.Values{}
	values.Set("file_id", request.FileID)
	...
	return c.downloadFile(ctx, envelope.Result.FilePath, request)
}

func (c *HTTPClient) downloadFile(ctx context.Context, filePath string, request DownloadRequest) (io.ReadCloser, error) {
	...
	for attempt := 0; attempt < 4; attempt++ {
		...
		if request.Offset > 0 || request.Limit > 0 {
			if request.Limit > 0 {
				req.Header.Set("Range", fmt.Sprintf("bytes=%d-%d", request.Offset, request.Offset+request.Limit-1))
			} else {
	...
	log.Printf("debug downloadFile offset=%d limit=%d fileURL=%s lastErr=%v", request.Offset, request.Limit, fileURL, lastErr)
	return nil, lastErr
}
...
func uploadedFileFromResult(fileType string, result uploadResult) (UploadedFile, error) {
	...
	case TypeVideo:
		return mediaToUploadedFile("video", result.Video, result)
	case TypeAudio:
		return mediaToUploadedFile("audio", result.Audio, result)
	case TypeAnimation:
		return mediaToUploadedFile("animation", result.Animation, result)
	case TypeDocument:
		return mediaToUploadedFile("document", result.Document, result)
	default:
		return UploadedFile{}, fmt.Errorf("unsupported telegram upload type %q", fileType)
	}
}

func mediaToUploadedFile(name string, media telegramFile, result uploadResult) (UploadedFile, error) {
	if media.FileID == "" {
		// Fallback for when Telegram morphs a document into audio/video
		if result.Audio.FileID != "" {
			media = result.Audio
		} else if result.Video.FileID != "" {
			media = result.Video
		} else if result.Animation.FileID != "" {
			media = result.Animation
		} else if result.Voice.FileID != "" {
			media = result.Voice
		} else if result.VideoNote.FileID != "" {
			media = result.VideoNote
		} else if len(result.Photo) > 0 {
			media = result.Photo[len(result.Photo)-1]
		} else {
			resultBytes, _ := json.Marshal(result)
			return UploadedFile{}, fmt.Errorf("telegram upload response missing %s: %s", name, string(resultBytes))
		}
	}
	return UploadedFile{
		FileID:       media.FileID,
