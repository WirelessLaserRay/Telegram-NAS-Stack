...
type ObjectStore interface {
	...
	CopyObject(ctx context.Context, bucket, srcKey, dstKey string) (store.CopyObjectResult, error)
...
func (s *Server) putObject(w http.ResponseWriter, r *http.Request, bucket, key string) {
	if copySource := r.Header.Get("X-Amz-Copy-Source"); copySource != "" {
		...
		s.logger.Printf("debug event=s3_copy_object_request src_bucket=%q src_key=%q dst_bucket=%q dst_key=%q", srcBucket, srcKey, bucket, key)
		result, err := s.store.CopyObject(r.Context(), srcBucket, srcKey, key)
		if err != nil {
			s.logger.Printf("debug event=s3_copy_object_result src_bucket=%q src_key=%q dst_bucket=%q dst_key=%q result=error error=%q", srcBucket, srcKey, bucket, key, sanitizeLogError(err))
			WriteErrorResponse(w, r, MapError(err), r.URL.Path, "")
			return
		}

		s.logger.Printf("debug event=s3_copy_object_result src_bucket=%q src_key=%q dst_bucket=%q dst_key=%q result=success", srcBucket, srcKey, bucket, key)
		copyResultXML := CopyObjectResultXML{
			LastModified: result.LastModified.UTC().Format(time.RFC3339),
			ETag:         fmt.Sprintf("\"%s\"", result.ETag),
		}
		writeXML(w, http.StatusOK, copyResultXML)
		return
	}
