...
func (f *FakeTelegram) Download(ctx context.Context, request telegram.DownloadRequest) (io.ReadCloser, error) {
	if f.DownloadFunc != nil {
		return f.DownloadFunc(ctx, request.FileID)
	}
	f.mu.Lock()
	f.Downloads = append(f.Downloads, request.FileID)
	data, ok := f.Files[request.FileID]
	f.mu.Unlock()
	if !ok {
		return nil, fmt.Errorf("fake telegram file %q not found", request.FileID)
	}
